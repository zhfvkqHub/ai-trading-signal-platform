...
public class KafkaConsumerConfig {
    ...
    public CommonErrorHandler kafkaErrorHandler(KafkaProperties kafkaProperties) {
        ...
        var errorHandler = getDefaultErrorHandler(producerProps);

        log.info("Kafka DLQ 에러 핸들러 초기화 완료");
        return errorHandler;
    }

    private static @NonNull DefaultErrorHandler getDefaultErrorHandler(
        ...
        var recoverer = new DeadLetterPublishingRecoverer(dlqTemplate,
                (kafka, ex) -> new TopicPartition(
                        kafka.topic() + ".dlq", kafka.partition()));
